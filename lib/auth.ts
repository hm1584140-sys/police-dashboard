...
async function verifyPassword(password: string, stored: string): Promise<boolean> {
  if (!stored.startsWith(`${PASSWORD_PREFIX}import { randomBytes, scrypt, timingSafeEqual } from 'node:crypto'
import { getServerSupabase } from '@/lib/supabase-server'
...
export type SessionInfo = {
  ...
  ban_reason?: string
    ...
    // Legacy plaintext password. Successful login upgrades it to a hash.
  ...
  if (!data.password.startsWith(`${PASSWORD_PREFIX}import { randomBytes, scrypt, timingSafeEqual } from 'node:crypto'
import { getServerSupabase } from '@/lib/supabase-server'
